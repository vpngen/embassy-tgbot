...
Если ты не помнишь имя — возможно стоит начать сначала и идти за новой картинкой.`
	...
	RestoreTrackInvalidNameMessageVIP = `Это не похоже на имя, попробуй еще раз. Будь внимателен(-на), тебе нужно ввести прилагательное и фамилию лауреата, например “Веселый Эйнштейн” или “Потрясающая Кюри”.  

Можно стартануть с чистого листа. Но есть нюанс: сейчас мы не можем выдать новую обычную бригаду — ресурсы закончились.
Зато ты можешь сразу взять VIP-бригаду 😎. Она твоя — без ограничений, ожиданий и удалений.`
...
А еще вспомни, когда ты в последний раз пользовался нашим VPN? Если это было давно - мы тебя удалили и пора идти за новой картинкой.`
	...
	RestoreTrackBrigadeNotFoundMessageVIP = `Сим-сим не открылся, с именем или словами что-то не так. Будь внимателен(-на), имя состоит из прилагательного и фамилии лауреата, например “Веселый Эйнштейн” или “Потрясающая Кюри”. А шесть слов должны быть ровно в той последовательности, в которой мы тебе их отдали!

Если ты не помнишь имя и/или волшебные слова — возможно стоит начать с чистого листа. Но есть нюанс: сейчас мы не можем выдать новую обычную бригаду — ресурсы закончились.
Зато ты можешь сразу взять VIP-бригаду 😎. Она твоя — без ограничений, ожиданий и удалений.`
...
Если ты в России/Беларуси и беспокоишься о своей безопасности — пожалуйста, установи автоудаление сообщений в этом чате на 1 день. [Инструкция](https://telegram.org/blog/autodelete-inv2/ru?ln=a)`
	...
	/*
		...
		• Начиная с восьмого дня после твоей регистрации иметь не менее десяти активных пользователей в месяц. Т.е. начать распространять VPN в своем окружении.

		Или мы удалим твою бригаду ` + "\U0001F937" + `.

		Но у тебя есть вариант помочь и себе, и нам:
		🧬 *VIP-Бригада*
		— до 250 ключей для друзей и семьи + уникальный IP, не попадающий в списки блокировок;
		— никакого удаления при нехватке числа пользователей;
		— быстрая и надежная поддержка.

		*Бонус бригадира* — 1 VIP-ключ для YouTube в 4K, видеоигр и стриминга.

		Твоя оплата поможет Генератору работать дальше.
		...
		`
	*/
	MainTrackWelcomeMessage = `🌐 VPN Generator – чтобы оставаться на связи с близкими даже тогда, когда всё вокруг пытаются отключить.

ВАЖНО: здесь ты получаешь целый сервер. Это ценный и ограниченный ресурс, поэтому есть простые правила:

• Начать раздавать VPN-ключи в течение 24 часов.
• С 8-го дня после регистрации иметь минимум 10 активных пользователей.

Или можешь подключить VIP-бригаду с повышенной скоростью и без ожиданий.
...
Нажми кнопку ниже ↓`
	...
	/*
		...
		Бесплатные бригады сейчас не выдаем — силы уходят на то, чтобы сотни тысяч людей каждый день имели доступ к свободному интернету с ранее выданными бригадами.
		...
		*Бонус бригадира* — 1 VIP-ключ для YouTube в 4K, видеоигр и стриминга.
		...
		Твоя оплата поможет Генератору работать дальше.
		Оставайся на связи! [Подпишись](https://t.me/vpngen) на новости проекта и борьбы с блокировками!`
	*/
	MainTrackWelcomeMessageVIP = `Привет!

Это VPN Generator — доступ в интернет для друзей, родных и всех своих.

Выбери одну из трех опций:

– «Хочу бригаду» — чтобы раздавать ключи VPN всем, кого знаешь
– «Куплю VIP-бригаду» — чтобы получить и раздавать супер-доступ: выше скорость VPN, VIP-поддержка и иммунитет от удаления
– «Восстановлю бригаду» — если ее удалили и ты помнишь секретные 6 слов и имя бригадира

Обязательно подпишись на @vpngen, чтобы узнавать о новых возможностях, обновлениях и важных новостях проекта.
Инструкция для бригадиров — [тут](https://bit.ly/VPNgenManual).

Есть вопрос или нужна помощь? Нажми «Помощь» — ответим.
Правда, не всегда быстро ☺
Для VIP — значительно быстрее.`
	...
	/*
		...
		_Если у тебя появятся вопросы — напиши нам в_ [поддержку](%s), _мы расскажем_ ` + "\U0000263A."
	*/

	mainTrackQuizMessage_one = `Круто, что ты решил стать VPN-бригадиром и раздавать доступ в свободный интернет! 

Пожалуйста, пришли любую картинку в этот чат: надо понять, что ты живой человек, а не тролль из-под моста. 
 
На следующем шаге ты получишь VPN-бригаду. Подробнее – в инструкции: https://bit.ly/VPNgenManual`

	// MainTrackSendForAttestationMessage - receipt have accepted.
	MainTrackSendForAttestationMessage = `Воу, какая картинка! Вся наша команда рассматривает ее и скоро ответит тебе.`

	// mainTrackWarnRequiredPhoto - warning about photo absents.
	mainTrackWarnRequiredPhoto = `Похоже ты забыл прикрепить картинку. Попробуй ещё раз. Если ты не помнишь условий, используй команду /repeat . ` + extraSupportText

	// mainTrackWarnWaitForApprovement
	mainTrackWarnWaitForApprovement = `Ожидай пока вся наша команда посмотрит на твою картинку. Это может занять какое-то время. Если у тебя остались вопросы — напиши пожалуйста в [поддержку](%s).`
	...
	MainTrackProto0ConfigMessage = "На всякий случай твой *личный* ключ Reality (так называется ещё один поддерживаемый нами протокол) ниже. Используй его со специальным приложением в случае, если ничего не работает. Скопируй его и добавь в приложение, которое ты выбрал(-а) для работы с Reality (например Hiddify)"
...
Спрячь эти слова туда, куда ты сможешь добраться в любой непонятной ситуации, но не доберется трщ майор. Нет, не туда! Туда доберется… Лучше в хранилку паролей или еще какое-нибудь хитрое место.`
	...
	*/
	...
	RestoreTrackConfigsMessage = `*Шаг 3.* Подключился? Тогда *обязательно* перейди по ссылке [http://vpn.works/](http://vpn.works/) *в течение 24 часов*, чтобы активировать VPN и свою бригаду. *Вы не сможете восстановить бригаду в течение 30 дней в случае если не зайдёте в ключницу в течение суток.*`
...
Если и конфигурация, и слова для восстановления потеряны, боюсь, мы бессильны тебе помочь.`
	...
	//
	// VIPMessage = `Покупка VIP-бригады происходит в специальном боте`
	VIPMessage = `Ты почти VIP-бригадир с максимальной защитой и скоростью! Переходи в бот для покупки VIP-бригады — всё быстро и без лишних шагов.`
)

var (
	// VIPBotURL - VIP bot URL.
	VIPBotURL string
...
const newTrack = "Хочу VIP бригаду"
...
func SetSupportMessages(url string) {
	...
	WannabeKeyboard = tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("Хочу бригаду", "started"),
			tgbotapi.NewInlineKeyboardButtonData("Куплю VIP-бригаду", "vip"),
			tgbotapi.NewInlineKeyboardButtonURL("Задать вопрос", url),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("Восстановлю бригаду", "restore"),
		),
	)
	...
	MainTrackQuizMessage["1_"] = mainTrackQuizMessage_one // fmt.Sprintf(mainTrackQuizMessage_one, link)
